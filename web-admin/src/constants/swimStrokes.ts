...
const LABEL_TO_CODE = new Map(
  SWIM_STROKE_OPTIONS.map((option) => [option.label, option.code])
);

export function strokeLabelToCode(label: string): string {
  return LABEL_TO_CODE.get(label) ?? label;
}
...
export function strokeLabelsToCodes(labels: string[]): string[] {
  return labels.map(strokeLabelToCode);
}
