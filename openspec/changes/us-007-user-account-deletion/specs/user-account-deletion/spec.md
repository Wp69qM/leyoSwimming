...
系统 MUST 提供 `POST /api/user/account/cancel` 接口，供已登录用户提交账号注销申请。系统 MUST 在注销前校验用户无 active 状态的套餐、无未完成或退款中订单。系统 MUST 要求用户完成二次验证（密码或验证码）。系统 MUST 在注销成功后将 `user.status` 更新为 `2`（已注销）、设置 `deleted_at` 与 `anonymous_after`（当前时间 + 90 天）、清除所有登录态、记录 `audit_log`。系统 MUST 对重复提交做幂等处理，返回已注销结果。
...
And   user.anonymous_after 为当前时间 + 90 天
And   登录态被清除
And   页面跳转至登录页
...
系统 MUST 提供 `GET /api/user/account/cancel/check` 接口，供已登录用户查询当前是否满足注销条件。系统 MUST 返回 active 套餐数量、未完成订单数量、是否可注销等字段。

#### Scenario: 查询满足注销条件

```gherkin
Given 用户已登录且无 active 套餐、无未完成订单
When  用户调用 GET /api/user/account/cancel/check
Then  返回 HTTP 200
...
```gherkin
Given 用户已登录且存在 1 个 active 套餐和 2 个未完成订单
When  用户调用 GET /api/user/account/cancel/check
Then  返回 HTTP 200
