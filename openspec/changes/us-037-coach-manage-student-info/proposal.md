...
- 教练可查看学员完整资料：US-005 自主档案（头像、姓名、手机号脱敏、年龄、性别、游泳基础等，只读）+ 教练视角切片
...
- 后端：新增 `coach_student_profile` 表、`/api/coach/v1/students` 系列接口、字段校验与加密逻辑；GET 详情接口需联查 `user` 表返回 US-005 字段
