...
## Task 7: 自动注册与已离职账号复用 RED
- 编写失败测试：未注册手机号登录后创建 status=-1 的新 coach 记录；已离职手机号登录后复用原记录并返回 coach_status=3。
- 运行测试确认失败。

## Task 8: 自动注册与已离职账号复用 GREEN
- 实现 `coach` 记录的查询/创建逻辑，未注册时新建、已离职时复用原记录。
- 运行测试确认通过。
