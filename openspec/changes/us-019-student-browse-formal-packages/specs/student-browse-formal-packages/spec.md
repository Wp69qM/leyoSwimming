...
系统 MUST 通过 `GET /api/coaches/{id}/packages` 向学员/游客返回指定教练的正价套餐选项。系统 MUST 仅返回 `coach.status IN (1, 4)` 的教练套餐；教练状态不属于可见集合时 SHALL 返回 HTTP 404 与错误码 `COACH_NOT_FOUND`。系统 MUST 返回启用的标准套餐列表（`standard_package.status = 1`）与教练参考单价；当教练未设置参考单价时 SHALL 将 `custom_package_enabled` 置为 `false`。

#### Scenario: 正常浏览正价套餐

```gherkin
Given 教练 A 状态为已通过（status=1）
And   管理员已配置 4 个启用的标准套餐（1/6/8/10 节）
...
```gherkin
Given 教练 D 状态为已通过（status=1）
And   教练 D 已设置参考单价
And   管理员尚未配置任何启用的标准套餐
