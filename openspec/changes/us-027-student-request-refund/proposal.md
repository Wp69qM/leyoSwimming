...
- **状态机**：order 已支付 → 退款审批中；package active/exhausted/expired → frozen(refund_pending)，释放 reserved_count → 0；package frozen(coach_resigned) → frozen(refund_pending)，保留 frozen_reason 历史值用于 100% 退款计算
