...
- 提交退款时事务内执行：创建 refund_record(status=待审批) + order.status → 退款审批中 + package.status 保持 active + package.booking_frozen = true
- 释放 package 的 reserved 课时，通过 booking_frozen 禁止审批期间新增预约
...
- **数据表**：新增 `refund_record`；修改 `order`（status → 退款审批中）、`package`（status 保持 active，booking_frozen = true）、`notification`（新增通知记录）
- **API**：新增 2 个学员侧端点（资格检查 + 退款提交）
- **状态机**：order 已支付 → 退款审批中；package active/exhausted/expired → active（保持），设置 booking_frozen=true；package frozen(coach_resigned) → active，保留 frozen_reason
