...
- [ ] **RED:** Write 4 failing tests — `findPublicList` returns published templates sorted by sortOrder ASC with experience first; filters out unpublished (status=0) and custom (package_type=2); filters out price=0 gift templates; returns empty array when no published templates
- [ ] **GREEN:** Implement `PackageTemplateRepository.findPublicList({ type })` — `WHERE status=1 AND package_type IN [0,1] AND price > 0 ORDER BY sort_order ASC, package_type ASC`
- [ ] **REFACTOR:** Extract `PUBLIC_PACKAGE_TYPES` constant, review WHERE clause ordering, normalize field alias naming
...
- [ ] **REFACTOR:** Extract `MAX_ANNOUNCEMENT_LIMIT` constant, verify time-window index usage
...
- [ ] **REFACTOR:** Extract closure type mapping helper, ensure null handling is explicit
...
- [ ] **RED:** Write 7 failing tests — `GET /packages` returns 200 with items array + 200 with empty array; `GET /announcements` returns 200 with items + 200 with empty array + 200 with clipped limit (max 10); `GET /venue` returns 200 with venue info + 404 with VENUE_NOT_CONFIGURED
- [ ] **GREEN:** Implement 3 controllers (`listPackages`, `listAnnouncements`, `getVenue`) + Koa router with `GET /api/v1/packages`, `GET /api/v1/announcements`, `GET /api/v1/venue` + limit clamping in repository + 404 error handling for venue
- [ ] **REFACTOR:** Extract shared limit validator, unify error response shape
