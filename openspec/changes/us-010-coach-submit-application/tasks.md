...
**Spec coverage:** REQ-001 Scenario "证书图片过大或格式错误"
...
- [ ] **RED:** Write 2 failing tests — 已有正式提交记录返回 `COACH_APPLICATION_PENDING`；已离职教练可重新提交
- [ ] **GREEN:** Add duplicate application check based on `status=0` and `submitted_at IS NOT NULL`
...
- [ ] **RED:** Write 3 failing tests — status=-1 首次保存草稿返回 200 且 `status=0`；`submitted_at` 为 NULL 表示草稿；status=2 保存草稿后 `rejection_reason` 清空
- [ ] **GREEN:** Implement `PUT /api/coach/application/draft`（保存草稿与提交审核均写入 `status=0`，不新增独立草稿状态）
...
**Spec coverage:** REQ-001 Scenario "已驳回教练重新提交入驻资料"

- [ ] **RED:** Write 2 failing tests — status=2 教练重新提交后 status=0；`rejection_reason` 清空且 `submitted_at` 更新
- [ ] **GREEN:** Implement resubmission logic for rejected applications
- [ ] **COMMIT:** `feat(coach): support resubmission after rejection`

## Task 8: 等待审核页查询完整资料 [P1]

**Files:**
- Modify: `backend/src/controllers/coach/application.ts`
- Create/Modify: `backend/src/repositories/coach_certificate.ts`
- Test: `backend/tests/controllers/coach/application.test.ts`

**Spec coverage:** REQ-003 Scenario "等待审核页查看已提交资料"

- [ ] **RED:** Write 2 failing tests — GET /api/coach/application 返回完整资料含证书列表；手机号与身份证号脱敏展示
- [ ] **GREEN:** Implement GET /api/coach/application with full data and certificate list
...
**Spec coverage:** REQ-001 / REQ-002 / REQ-003 正常/异常场景
