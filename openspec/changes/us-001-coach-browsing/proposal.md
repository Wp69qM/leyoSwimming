...
- 新增 `POST /api/coach/list` 公开列表接口（无需登录，按综合评分降序 + 分页，参数通过 JSON body 传递）
- 新增 `POST /api/coach/detail` 公开详情接口（含个人形象照、年龄、性别、学员总数、总课时数、证书、简介、参考单价、联系方式、可选套餐、学员评价、可约时间预览、实时状态等完整字段，参数通过 JSON body 传递）
- 新增微信小程序「找教练」列表页（支持空状态、下拉刷新、无限滚动）
- 新增微信小程序教练详情页（含实时状态徽标、"立即预约"按钮状态显隐、可选套餐默认 3 个卡片 + 「更多」入口、可约时间预览 + 「查看全部」入口）
