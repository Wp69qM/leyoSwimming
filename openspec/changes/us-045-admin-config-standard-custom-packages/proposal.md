...
- 新增 `custom_package_config` 表，存储自定义套餐全局规则（允许课时范围 min/max、默认有效期、单价下限）
- 新增 7 个管理员接口：标准套餐列表、新增、编辑、上下架切换、详情、自定义套餐规则配置、图片上传
...
- **API**：新增 7 个管理员接口（标准套餐列表/新增/编辑/上下架切换/详情 + 自定义套餐规则配置 + 图片上传）
