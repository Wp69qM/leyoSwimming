...
PRD [§5.2.1](../../../docs/prd/prd.md) 要求支持微信授权登录，首次登录后补充手机号、用户名、密码、邮箱；[§9.2 身份体系](../../../docs/prd/prd.md) 定义了用户身份状态机：游客 → 注册用户（通过微信授权登录）→ 学员（购买正价套餐后）。

## What Changes

- 新增 `POST /api/user/auth/wechat-login` 接口（接收微信 `code`，调用 `code2session`，查询/创建用户，签发 JWT）
...
- 支持两种触发路径：游客首次访问受登录限制功能时系统引导跳转；游客在「我的」页面发现未登录态（默认头像+「请登录」按钮）主动点击「请登录」按钮跳转
- 登录成功后前端存储 `access_token`/`refresh_token`，后续请求通过 `Authorization: Bearer {access_token}` 携带，后端校验 token 维持登录态
...
- **API**：新增 1 个写入端点 `POST /api/user/auth/wechat-login`（无需登录鉴权）
...
- **依赖**：无前置 US（本 US 是用户身份流程起点）；被 US-005~US-009、US-017、US-020、US-042、US-051 依赖
