...
系统 MUST 允许 `coach.status = 3`（已离职）的教练发起重新入驻。已离职教练登录教练端时，US-051 / US-054 MUST 按 `coach_status = 3` 直接跳转 US-010 的 C-入驻资料填写页。实际资料填写、校验与提交 MUST 由 US-010 处理；US-010 MUST 为重新入驻创建 `coach_application` pending 快照，`previous_coach_status = 3`，并将 `coach.status` 从 3 更新为 0。系统 MUST 在历史数据不隔离、不回滚的前提下复用原 `coach` 记录。
...
#### Scenario: 非已离职教练无法通过登录进入重新入驻

```gherkin
Given 教练 C 的 coach.status = 1（已通过）
When  教练 C 登录教练端（US-051 / US-054）
Then  系统返回 coach_status = 1
And   前端按 US-051 / US-054 正常映射跳转（例如教练首页）
...
When  教练 C 再次进入 US-010 的 C-入驻资料填写页并点击「提交审核」
Then  US-010 重复提交校验返回错误码 COACH_APPLICATION_PENDING
And   coach.status 保持 0，不产生新的待审核记录
```

### Requirement: REQ-002 管理员可通过重新入驻审核

系统 MUST 允许管理员通过重新入驻申请。审核通过后，`coach_application` 快照字段 MUST 覆盖写入 `coach` 表生效资料，`coach.status` MUST 从 0 更新为 1，教练恢复教学资格。历史评分/评价 MUST 保留，并对新老学员均可见。
...
系统 MUST 阻止 `coach.status ≠ 3` 的教练提交重新入驻申请。US-051 / US-054 登录响应中的 `coach_status` 不为 3 时，前端 MUST 不进入 US-010 C-入驻资料填写页。

#### Scenario: 已通过教练登录后看不到重新入驻流程
...
系统 MUST 确保教练在已存在 pending 状态的 `coach_application` 时无法再次提交重新入驻申请。US-010 的提交接口 MUST 在检测到已有 pending 申请时返回 `COACH_APPLICATION_PENDING`。
...
Given 教练 C 的 coach.status = 0（待审核）
And   存在 coach_application.status = pending 且 previous_coach_status=3 的申请
When  教练 C 在 US-010 的 C-入驻资料填写页点击「提交审核」
Then  US-010 返回错误码 COACH_APPLICATION_PENDING
