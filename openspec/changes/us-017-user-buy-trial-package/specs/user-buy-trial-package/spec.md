...
系统 MUST 允许已登录用户购买体验课套餐。系统 MUST 校验用户已阅读并同意最新版《用户须知》、《健康承诺书》、《免责协议》（PRD §6.9 / §11.3；隐私协议在 US-004 已签署）、用户名下无 active/exhausted 体验套餐且教练状态为已通过；支付成功后 MUST 创建 active 体验套餐、记录协议签署并将用户身份升级为学员。
...
Given 游客已登录且名下无体验套餐
And   教练 A 状态为已通过（status=1）
And   用户未勾选《用户须知》《健康承诺书》《免责协议》中任一项
When  游客提交体验课订单
