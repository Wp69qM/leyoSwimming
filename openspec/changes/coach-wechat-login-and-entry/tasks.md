...
- [ ] **RED:** Write failing tests — `app_type=coach` 且无 coach 记录返回 `redirect_page=coach_onboarding` 且 `is_new_coach=true`; `coach.status=1` 返回 `coach_home` 且 `is_new_coach=false`; `coach.status=0` 返回 `coach_pending`; `coach.status=2` 返回 `coach_rejected` 并带 `rejection_reason`; `coach.status=3` 视为未命中并新建 coach; 未勾选协议抛 `TERMS_NOT_ACCEPTED`; 非法 `app_type` 抛 `VALIDATION_ERROR`; 不调用 user repo。
- [ ] **GREEN:** Extend `WechatAuthService.authenticate(...)` — when `appType='coach'`, validate terms/privacy, call `CoachRepository.findCoachByUnionId`, create coach if needed, compute `redirect_page`, return `is_new_coach`/`coach_status`/`redirect_page`; do not touch `user` repo。
- [ ] **REFACTOR:** Extract `resolveCoachRedirectPage(coachStatus)` pure function; extract terms/privacy validation。
...
- [ ] **RED:** Write failing tests — `app_type=coach` 未入驻返回 200 + redirect_page; `app_type=coach` 已通过返回 200 + coach_home; `app_type=coach` 缺失手机号加密数据返回 400; `app_type=coach` 未勾选协议返回 400 + TERMS_NOT_ACCEPTED; `app_type=user` 保持 US-004 响应格式；非法 `app_type` 返回 400；幂等返回首次结果；拒绝授权/微信错误按 US-004 处理。
...
- [ ] **RED:** Write failing tests — 已登录已通过教练返回 coach_status=1 + coach_home; 已登录无 coach 记录返回 coach_status=-1 + coach_onboarding; 已登录已驳回教练返回 rejection_reason; 未登录返回 401。
- [ ] **GREEN:** Implement `GET /api/v1/coach/me/status` with auth middleware (coach_id from JWT), query coach by coach_id, return redirect_page。
...
- [ ] **GREEN:** Implement `CoachLoginPage` — check terms/privacy → call `Taro.login()` → `wx.getPhoneNumber()` → POST `/auth/wechat-login` with `app_type=coach` → save tokens → redirect by `redirect_page`; implement route guard to call `/coach/me/status` when token exists and handle refresh/relogin。
