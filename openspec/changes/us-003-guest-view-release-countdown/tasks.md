...
- [ ] **REFACTOR:** 检查字段别名映射统一性，提取可复用查询片段，确保 `findActive` 语义清晰
...
- [ ] **REFACTOR:** 提取时间格式化与日期比较纯函数，将窗口判断、文案生成分解为独立方法，消除重复返回对象
...
- [ ] **REFACTOR:** 抽取 DB 时间与 Redis 缓存逻辑到公共工具，统一日志级别并验证序列化语义
...
- [ ] **REFACTOR:** 提取定时器清理 hooks，将内联样式抽取为组件样式，确保释放当天固定文案与动态剩余时间一致
