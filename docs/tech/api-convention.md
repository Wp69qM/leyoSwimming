...
| 模块 | 前缀 | 示例 |
|------|------|------|
| 通用 | `COMMON_` | `COMMON_001` 参数错误 |
| 认证授权 | `AUTH_` | `AUTH_001` Token 失效 |
| 教练 | `COACH_` | `COACH_001` 教练不存在 |
| 教练入驻申请 | `COACH_APPLICATION_` | `COACH_APPLICATION_001` 申请不存在 |
| 管理员 | `ADMIN_` | `ADMIN_001` 无管理员权限 |
| 订单 | `ORDER_` | `ORDER_001` 订单不存在 |
...
|--------|------|
| `COMMON_001` | 请求参数错误 |
| `COMMON_002` | 请求体 JSON 解析失败 |
| `COMMON_003` | 必填参数缺失 |
| `AUTH_001` | 未登录或 Token 无效 |
| `AUTH_002` | 登录已过期 |
| `FORBIDDEN_001` | 无操作权限 |
| `SYSTEM_001` | 系统内部错误 |
