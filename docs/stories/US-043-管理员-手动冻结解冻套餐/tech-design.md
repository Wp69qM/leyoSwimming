...
| 字段 | 类型 | 索引 | 备注 |
|------|------|------|------|
| status | TINYINT | IDX | 0=active 1=frozen ... |
| frozen_reason | TINYINT | IDX | 0=pending_review 1=admin_manual 2=court_order |
| reserved_count | INT | | |
| available_count | INT | | |
| version | INT | | 乐观锁 |
...
- **请求体**：`{ "reason": "pending_review", "version": 1 }`
...
- `已预约` / `待上课` → `已取消`（冻结时，cancel_reason = package_frozen）
