...
- [ ] **4.1 RED**：编写 expired package 延期成功测试
- [ ] **4.2 RED**：编写 active package 延期更新 expire_at 测试
- [ ] **4.3 RED**：编写 exhausted package 延期失败测试
- [ ] **4.4 GREEN**：实现 `POST /api/admin/package/extend`
- [ ] **4.5 COMMIT**：`feat(us-043): admin extend package`

**对应**：user-story.md 场景 6.5 / tech-design §4.5

### Task 5：发起退款接口

- [ ] **5.1 RED**：编写 active package 发起退款成功测试
- [ ] **5.2 RED**：编写 refund_enabled=false 发起退款失败测试
- [ ] **5.3 RED**：编写重复发起退款失败测试
- [ ] **5.4 GREEN**：实现 `POST /api/admin/package/refund`
- [ ] **5.5 COMMIT**：`feat(us-043): admin request package refund`
...
| 5 | 管理员手动延期套餐 | 6.5 | `test_admin_extend_package_success` | expired → active，expire_at 更新 |
| 6 | 管理员发起退款 | 6.6 | `test_admin_request_refund_success` | 创建 refund 订单，package=frozen/refund_pending |
...
| 13 | 并发冻结/解冻/延期 | 8.1 | `test_concurrent_package_status_change` | 仅一次成功 |
| 14 | 冻结时取消未上课 booking | 8.2 | `test_freeze_cancels_future_bookings` | 未来 booking 取消 |
| 15 | 退款驳回后套餐恢复 active | 8.5 | `test_reject_refund_restores_package` | frozen/refund_pending → active |
