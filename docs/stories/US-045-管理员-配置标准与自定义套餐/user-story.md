...
> 本 US 复杂度已提升，场景数 = 3 正常 + 6 异常 + 6 边界 = 15（待测试计划细化）。
...
When  管理员提交标准套餐：名称="暑期 10 节课", 教学类型="一对一", 泳姿=[], 课时数=10, 每节课时长=60, 有效期=180 天, 原价=3600.00, 售价=3000.00, 是否支持退款=true, 退款比例=80, 退款有效期限制=30, 标签=["热销"], 套餐描述="<p>暑期特惠套餐</p>", 套餐展示图片=["https://cdn.example.com/a.jpg"], 状态=未上架
Then  系统返回 HTTP 200 且 package_template 表新增 1 条记录
And   该记录 status='inactive'，total_hours=10，duration_minutes=60，original_price=3600.00，price=3000.00
...
- **触发条件**：套餐处于 `active` 状态，管理员编辑价格或课时数
- **预期行为**：系统允许编辑并自动下架（或二次确认后下架），避免已上架套餐参数突变影响新购用户
- **用户可见反馈**：弹窗提示"修改已上架套餐会自动下架，需重新手动上架"
...
- 列表/详情提供「上架」按钮；已上架后变为「下架」按钮
- 上架/下架按钮点击后二次确认
- 生效后立即影响游客端/学员端套餐列表
- 编辑已上架套餐时，保存后自动下架（需二次确认）
...
| 查看详情 | 点击行/按钮 | 打开详情抽屉/弹窗 | 可执行上架/下架/编辑 |
