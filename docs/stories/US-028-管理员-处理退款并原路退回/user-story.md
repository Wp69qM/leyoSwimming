...
| 2 | 退款金额依据 package 快照 `paid_amount × (total_hours - consumed_count) / total_hours × refund_ratio` 计算，不受后续 package_template 变更影响；管理员可在 `[0, 可退金额]` 范围内修改 | [§6.4.1](../../prd/prd.md)、[§6.4.2](../../prd/prd.md) |
...
### 8.3 边界场景 3：管理员修改后的退款金额超过可退金额

- **触发条件**：管理员在通过退款时将金额修改为超过可退金额
- **预期行为**：审批前校验失败，返回 REFUND_AMOUNT_MISMATCH，拦截并提示管理员
- **用户可见反馈**：后台弹窗"退款金额超过可退金额，请核对"
...
- **金额调整**：管理员可在 `[0, 可退金额]` 范围内修改退款金额；修改原因与调整记录写入 audit_log
