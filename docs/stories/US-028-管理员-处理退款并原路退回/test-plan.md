...
- [ ] **RED:** Write failing tests — approve refund updates order to 退款处理中 then 已退款 on channel success, package.status frozen(refund_pending)→refunded, creates transaction; reject refund sets order.status=退款被拒（7）, restores package.status to active and booking_frozen=false; channel failure keeps package.status frozen(refund_pending) and booking_frozen=true; idempotent duplicate approval; forbid non-admin
