...
- **触发方**：游客/未购买正价套餐的注册用户
- **触发动作**：
  - **入口 1（首页体验课卡片）**：首页展示体验课套餐卡片 → 点击进入体验课套餐详情页 → 点击底部「立即购买」→ 进入确认订单页（展示套餐名称、价格、类别、协议勾选）→ 勾选协议后提交订单 → 进入支付方式选择 → 确认支付
  - **入口 2（全部套餐列表）**：首页点击「全部套餐」→ 进入套餐列表页 → 体验课卡片按展示规则呈现 → 后续流程同入口 1
  - **入口 3（教练详情页可选套餐）**：在教练详情页点击「可选套餐」中的体验课卡片 → 后续流程同入口 1
- **触发时机**：游客/未购课用户主动转化时
...
- [x] 用户已签署《健康承诺书》与《免责协议》（§11.3，首次购课前一次性签署，长期有效）
...
1. 游客通过任一入口（首页体验课卡片 / 全部套餐列表 / 教练详情页可选套餐）进入体验课套餐详情页，点击「立即购买」
2. 系统进入确认订单页，展示套餐名称、价格、类别及《用户须知》《健康承诺书》《免责协议》，并校验用户已阅读并同意最新版《用户须知》，且已签署《健康承诺书》《免责协议》（首次购课前需完成签署）
...
- **分支 1**：用户未阅读并同意《用户须知》《健康承诺书》《免责协议》中任一项 → 返回错误 `AGREEMENT_REQUIRED`
...
| 6 | 首次购课前必须签署《健康承诺书》 | [§11.3.1](../../prd/prd.md) |
| 7 | 首次购课前必须签署《免责协议》 | [§11.3.2](../../prd/prd.md) |
| 8 | 体验课模板必须已上架（`status='active'`）且 `package_mode='experience'` 才能购买 | US-045 |
| 9 | 已购套餐保存模板快照字段，不受后续模板变更影响 | US-045 |
| 10 | 体验课卡片展示规则：用户已购买正价套餐（含 active 正价套餐）时，首页/套餐列表/教练详情页不再展示体验课入口；用户未购买任何套餐时才展示 | 本 US 新增 |
...
| 4 | `payment` | 新增 | 支付流水 |
| 5 | `agreement_sign` | 新增 | 记录《用户须知》《健康承诺书》《免责协议》签署版本与时间 |
...
- [x] 引用 §5.2.3 / §4.1 / §4.5 / §6.2 / §3.3 / §11.3
- [x] 与状态机一致
...
- **性能要求**：下单接口 P99 < 300ms
- **协议签署**：下单前必须校验用户已同意最新版《用户须知》且已签署《健康承诺书》《免责协议》；支付回调成功时写入 `agreement_sign`
