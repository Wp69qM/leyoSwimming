...
4. 系统创建 active 体验套餐（package.status = active，available=1，expire_at=30 天后）
...
When  系统定时任务扫描超时订单
Then  订单状态变为已取消
And   对应 package.status 回滚为 refunded 或删除
...
| # | 表名 | 操作 | 说明 |
|---|------|------|------|
| 1 | `order` | 新增 | 体验课订单，course_type=0 |
| 2 | `package` | 新增 | 体验套餐，status=active，expire_at=30 天后 |
| 3 | `payment` | 新增 | 支付流水 |
| 4 | `user` | 修改 | identity 字段更新为学员 |

### 7.2 API 影响

| # | API | 方法 | 操作 | 说明 |
|---|-----|------|------|------|
| 1 | `/api/orders/trial` | POST | 新增 | 创建体验课订单 |
| 2 | `/api/orders/{id}/pay` | POST | 新增 | 发起支付 |
| 3 | `/api/payments/callback` | POST | 新增 | 支付回调 |

### 7.3 状态机影响

| # | 实体 | 转换 | 触发条件 | 说明 |
|---|------|------|---------|------|
| 1 | `order` | 无 → 待支付 → 已支付 | 提交订单 / 支付回调 | |
| 2 | `package` | 无 → active | 订单创建（预创建）或支付成功 | 详见 §4.5 |
| 3 | `user` | 注册用户 → 学员 | 支付成功 | |
...
- **事务边界**：订单创建 + 套餐预创建在同一事务；支付回调更新订单 + 激活套餐 + 身份更新在同一事务
- **性能要求**：下单接口 P99 < 300ms
