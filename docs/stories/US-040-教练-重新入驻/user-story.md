...
- **触发方**：教练
- **触发动作**：在教练端登录后点击「重新入驻」，进入 US-010 的入驻资料填写页
- **触发时机**：用户主动操作
...
1. 已离职教练登录教练端（US-051 按 `coach.status = 3` 直接跳转 US-010 的 C-入驻资料填写页，顶部展示重新入驻说明条）
2. 或者，已离职教练在「我的」页面看到「重新入驻」入口
3. 教练点击「重新入驻」
4. 系统校验 coach.status = 3，允许进入 C-入驻资料填写页（US-010）
5. C-入驻资料填写页自动回显 coach 表历史内容或最新 rejected/draft coach_application 快照，教练可编辑确认
6. 教练点击「提交审核」，由 US-010 处理资料校验与提交，创建新的 coach_application pending 快照（previous_coach_status=3），coach.status 从 3 更新为 0（待审核）
7. 管理员在后台审核重新入驻申请（复用 US-011 审核能力，审核视图中通过 previous_coach_status=3 标记为"重新入驻"）
8. 管理员审核通过后，coach_application 快照覆盖写入 coach 表，`coach.status` 从 0 更新为 1（已通过）

### 4.2 异常分支

- **分支 1**：教练状态非 3 → 隐藏「重新入驻」入口，点击时提示「当前状态不可重新入驻」
- **分支 2**：教练已处于 status=0（待审核） → 由 US-010 的重复提交校验处理，提示「入驻申请审核中，请勿重复提交」
- **分支 3**：重新入驻审核被拒绝 → `coach_application.status = rejected` 并写入 `rejection_reason`；`coach.status` 恢复为 3；coach 表生效资料保持不变，教练可再次进入 US-010 填写页重新提交
...
| 5 | 历史评分/评价保留但只对老学员可见，不对新学员展示 | [§5.4.8](../../prd/prd.md) |
...
| 4 | `coach_rating` / `review` | 只读 | 历史评分保留，新学员不可见 |
...
| # | API | 方法 | 操作 | 说明 |
|---|-----|------|------|------|
| 1 | `/api/coach/v1/reapply/entry` | POST | 新增/修改 | 已离职教练点击「重新入驻」后的入口校验；仅校验 `coach.status = 3` 并返回进入 US-010 填写页所需数据，不直接修改 coach.status |
...
- **触发条件**：教练快速多次点击「重新入驻」
- **预期行为**：入口校验幂等，仅允许进入一次 US-010 填写页；实际提交幂等由 US-010 处理
- **用户可见反馈**：仅一次成功提示
...
- **事务边界**： coach.status 的修改由 US-010 提交接口负责；US-040 的入口校验不修改 coach 状态
- **性能要求**：重新入驻入口校验接口 P99 < 150ms；审核状态查询可复用 US-011
- **历史数据**：`coach_rating.is_visible_to_new` 字段用于控制新学员可见性，重新入驻时保持为 false；coach 表历史数据不隔离、不回滚
...
| # | 内容 | 链接 / node-id | 状态 |
|---|------|---------------|------|
| 1 | 教练端「我的」页面「重新入驻」入口 Figma file URL | 🔲 待设计填写 | 🔲 待设计填写 |
| 2 | C-入驻资料填写页（status=3 重新入驻说明条状态）Figma file URL | 🔲 待设计填写 | 🔲 待设计填写 |
| 3 | 管理员后台重新入驻审核列表/详情页 Figma file URL | 🔲 待设计填写 | 🔲 待设计填写 |
...
|------|--------|---------|---------|---------|------|
| **「我的」页面重新入驻入口** | — | 🔲 | 🔲 | 🔲 | 仅 status=3 可见 |
| **C-入驻资料填写页（status=3）** | — | 🔲 | 🔲 | 🔲 | 顶部含重新入驻说明条；实际设计由 US-010 负责 |
...
- **背景**：重新入驻后若直接展示旧评分，可能误导新学员；但用户要求不做数据隔离
- **选项**：A. 完全删除历史评分；B. 保留在 coach 记录下但仅老学员可见
...
| 交互 | 触发 | 反馈 | 备注 |
|------|------|------|------|
| 登录后自动跳转 | US-051 状态分流 | 直接进入 US-010 C-入驻资料填写页 | status=3 时顶部展示重新入驻说明条 |
| 点击重新入驻 | 我的页面按钮 | 进入 US-010 C-入驻资料填写页 | status=3 时才可触发 |
| 提交资料 | US-010 资料页按钮 | 跳转审核状态页 | status 由 US-010 从 3 变为 0 |
| 审核通过 | 管理员操作 | 教练端收到通知 | status 变为 1 |
| 审核拒绝 | 管理员操作 | 教练端显示原因 | status 恢复 3，可再次进入 US-010 提交 |
