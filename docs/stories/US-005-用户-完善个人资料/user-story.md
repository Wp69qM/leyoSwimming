...
| **标题** | 用户完善个人资料 |
| **角色（Actor）** | 用户（主）、系统（辅）|
| **业务价值（Why）** | 用户首次登录后或在「我的 → 个人资料」中完善头像、姓名、年龄、性别等学员档案，使教练和后台可识别学员，并为用户推荐合适的课程与教练 |
| **优先级** | [MVP] |
| **估时** | 0.5 人天 |
...
4. 用户选填：有无游泳基础、会什么泳姿（有基础时显示）、游泳年限（有基础时显示）、个人描述
5. 用户点击「保存」
6. 系统校验必填项、字段格式、手机号唯一性、姓名敏感词
7. 系统保存资料，设置 `user.profile_completed = true`
8. 系统返回首页或「我的」页面（首次完善跳转首页，编辑资料返回「我的」）
...
| # | 表名 | 操作 | 说明 |
|---|------|------|------|
| 1 | user | 修改 | 补充/更新 avatar_url、name、phone、age、gender、has_swim_basis、swim_strokes、swim_years、personal_desc、profile_completed = true、updated_at |
...
- **影响范围**：表单设计、手机号唯一性校验、US-008 账号安全设置
