...
- [ ] **2.1 RED**：编写处理结果未全登记时校验失败测试
- [ ] **2.2 RED**：编写未来排班未清空时校验失败测试
- [ ] **2.3 RED**：编写教练费未结算时校验失败测试（可选）
- [ ] **2.4 GREEN**：实现 checklist 校验服务
- [ ] **2.5 COMMIT**：`feat(us-041): resignation approval checklist validation`
...
- [ ] **3.2 RED**：编写未来 booking 取消测试
- [ ] **3.3 RED**：编写 package active→frozen 测试
- [ ] **3.4 RED**：编写 schedule_slot hidden 测试
- [ ] **3.5 GREEN**：实现 approve 接口与批量更新事务
- [ ] **3.6 REFACTOR**：抽取 batch resignation processor
- [ ] **3.7 COMMIT**：`feat(us-041): approve resignation with batch updates`
...
|---|---------|--------------|---------|---------|
| 1 | 管理员通过离职审批 | 6.1 | `test_approve_resignation_success` | coach.status=3，booking/package/slot 批量变更 |
| 2 | 管理员拒绝离职审批 | 6.2 | `test_reject_resignation_success` | coach.status=1，action 保留 |
| 3 | 处理结果未全部登记 | 6.3 | `test_approve_checklist_not_passed` | HTTP 400，错误码 CHECKLIST_NOT_PASSED |
