...
- **触发方**：学员/注册用户
- **触发动作**：在支付页选择支付方式并点击「确认支付」，由系统调用 Mock 支付渠道模拟微信/支付宝支付
...
1. 用户进入订单支付页，系统展示订单金额、支付方式（微信/支付宝 Mock）
2. 用户选择支付方式并点击「确认支付」
3. 系统校验订单状态为待支付且未过期
4. 系统创建 payment 流水（channel = 0/1），并调用 `MockPaymentProvider.pay(...)` 模拟渠道支付，生成 `channel_trade_no`
5. Mock 渠道即时返回支付成功，并异步回调系统支付接口（或前端轮询订单状态）
6. 系统幂等处理回调：order.status → 已支付，paid_at 写入时间戳
7. 系统使用订单中已快照的模板字段创建 package，package.status → active（不实时查询 package_template）
8. 系统触发身份重算：注册用户 → 学员
9. 系统发送支付成功通知

### 4.2 异常分支

- **分支 1**：用户取消支付或 Mock 渠道返回失败 → order.status 保持待支付，payment.status = 失败，提示"支付失败，请重试"
...
| # | 规则 | 章节 |
|---|------|------|
| 1 | 购买套餐生成订单（order.type = purchase），支持微信/支付宝 Mock 支付；MVP 不调用真实支付 SDK | [§5.3.4](../../prd/prd.md) |
...
```gherkin
Given 用户已登录且存在待支付订单 order.type = purchase，order.status = 待支付，amount = 1800 元
...
And   订单未超过 24h 有效期
When  用户选择微信支付并点击「确认支付」
Then  系统创建 channel = 0 的 payment 流水，并调用 Mock 支付渠道
And   系统收到 Mock 支付成功回调
...
```gherkin
Given 用户已登录且存在待支付订单 order.type = purchase，order.status = 待支付，amount = 2400 元
...
And   订单未超过 24h 有效期
When  用户选择支付宝支付并点击「确认支付」
Then  系统创建 channel = 1 的 payment 流水，并调用 Mock 支付渠道
And   系统收到 Mock 支付成功回调
...
| # | 表名 | 操作 | 说明 |
|---|------|------|------|
| 1 | `order` | 修改 | status → 已支付，paid_at 回填；order.type = purchase，用于与退款订单区分（US-046）；order 中已保存模板快照字段，用于后续 package 创建 |
...
- **性能要求**：支付回调处理 P99 < 500ms，Mock 支付请求 P99 < 200ms
- **对账机制**：每日凌晨与第三方支付渠道对账，差异进入异常队列；MVP 阶段 Mock 渠道不产生真实对账差异
...
- 仅展示已配置的支付方式（微信/支付宝）
- MVP 阶段点击「确认支付」后调用后端 Mock 支付接口，不真正调起微信/支付宝 SDK；支付成功/失败由后端 Mock 渠道回调驱动
