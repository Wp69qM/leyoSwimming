...
INPUT_DIR = os.path.join(os.path.dirname(os.path.dirname(os.path.abspath(__file__))), "tmp", "calicat_user")
...
def hex_from_rgba(rgba):
    """Convert rgba(r,g,b,a) or #RRGGBB to #RRGGBB or #RRGGBBAA"""
    if not rgba:
        return None
...
def main():
    ...
    out_lines = ["# Calicat User First Batch Design Summary\n"]
    for fname in files:
