...
ALTER TABLE `coach` ADD COLUMN `email` VARCHAR(128) DEFAULT NULL COMMENT '邮箱';
ALTER TABLE `coach` ADD COLUMN `wechat_qr_url` VARCHAR(512) DEFAULT NULL COMMENT '微信二维码图片 URL';
ALTER TABLE `coach` ADD COLUMN `id_card_no` VARCHAR(128) DEFAULT NULL COMMENT 'AES 加密身份证号';
ALTER TABLE `coach` ADD COLUMN `total_students` INT DEFAULT NULL COMMENT '总学员数 0-99999';
ALTER TABLE `coach` ADD COLUMN `total_hours` INT DEFAULT NULL COMMENT '总课时数 0-99999';
ALTER TABLE `coach` ADD COLUMN `teaching_strokes` VARCHAR(128) DEFAULT NULL COMMENT '擅长泳姿，逗号分隔';
ALTER TABLE `coach` ADD COLUMN `bio` VARCHAR(500) DEFAULT NULL COMMENT '个人简介 10-500 字符';
ALTER TABLE `coach` ADD COLUMN `reference_price` DECIMAL(10, 2) DEFAULT NULL COMMENT '参考单价（元/节）50-2000';
ALTER TABLE `coach` ADD COLUMN `approved_at` DATETIME DEFAULT NULL COMMENT '最近一次审核通过时间';
